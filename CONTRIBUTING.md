...
- [How to Report Bugs](#how-to-report-bugs)
  * [Removing Mods for Debugging](#removing-mods-for-debugging)
- [How to Suggest Features](#how-to-suggest-features)
  * [Stay Polite](#stay-polite)
  * [Consider the workload](#consider-the-workload)
  * [Gather feedback from the community](#gather-feedback-from-the-community)
    + [Make sure that the feature is wanted](#make-sure-that-the-feature-is-wanted)
  * [Consider the Impact](#consider-the-impact)
    + [Why can't you just add a toggle](#why-can-t-you-just-add-a-toggle)
  * [What not to suggest](#what-not-to-suggest)
  * [Where to suggest a feature](#where-to-suggest-a-feature)
  * [Learn to accept a No](#learn-to-accept-a-no)
- [How to Contribute](#how-to-contribute)
  ...
  * [Setup guide](#setup-guide)
    ...
    + [Debugging](#debugging)
      - [Downloading the Unity player](#downloading-the-unity-player)
      - [Creating a KSP dev install](#creating-a-ksp-dev-install)
      - [Build and debug in Visual Studio](#build-and-debug-in-visual-studio)
...
All bugs should be reported on GitHub, under this URL: https://github.com/Kerbalism/Kerbalism/issues
...
Note that the following steps are specific to Windows and Visual Studio 2017+.
The steps here are specific to Windows and Visual Studio 2017+ but they should be more or less similar on OSX / Linux. There is some information available at this [KSP forums thread](http://forum.kerbalspaceprogram.com/index.php?/topic/102909-ksp-plugin-debugging-and-profiling-for-visual-studio-and-monodevelop-on-all-os/&page=1)

Note that live debugging on OSX / Linux is only possible since KSP 1.8

To be able to place breakpoint and use the debugging functions of Visual Studio, you need to :
...
If the *"WindowsPlayer"* process doesn't show up in this menu, check that both KSP and Visual Studio are allowed to communicate through the local firewall.

You should now be able to create breakpoints, step through the execution and inspect variables.
If that doesn't happen (the debugger just doesn't halt where you want it to), make sure that the debugging symbols (.mdb or .pdb depending on your KSP version) are available in the GameData directory, along with the Kerbalism.dll file.
