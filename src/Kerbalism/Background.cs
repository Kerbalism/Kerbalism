	...
	public static class Background
	{
		...
		public static void Update(Vessel v, VesselData vd, VesselResources resources, double elapsed_s)
		{
			...
			foreach (var e in pms)
			{
				switch(e.type)
				{
					...
					case Module_type.Experiment: (e.module_prefab as Experiment).BackgroundUpdate(v, vd, e.p, e.m, ec, resources, elapsed_s); break; // experiments use the prefab as a singleton instead of a static method
		...
		*/
		...
		static void ProcessRadioisotopeGenerator(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m, PartModule radioisotope_generator, ResourceInfo ec, double elapsed_s)
		{
			...
			double mission_time = v.missionTime / Lib.YearSeconds;
