	...
	public sealed class MiniHijacker : MonoBehaviour
	{
		void Start()
		{
			...
			while (dialog.pages.Count > 0)
			{
				...
				{
					DriveData drive = DriveData.SampleDrive(meta.vessel.KerbalismData(), data.dataAmount, meta.subjectData);
					if (drive != null)
		...
		internal static double RecordData(ScienceData data, MetaData meta)
		{
			...
			foreach(var drive in DriveData.GetDrives(meta.vessel.KerbalismData(), false))
			{
	...
	public sealed class Hijacker : MonoBehaviour
	{
		...
		void Record(MetaData meta, ScienceData data, bool send)
		{
			...
			if (recorded)
			{
				...
				if (!meta.is_sample && send)
				{
					foreach(var d in DriveData.GetDrives(meta.vessel))
