...
namespace KERBALISM
{
	...
	public class Laboratory: PartModule, IModuleInfo, ISpecifics, IContractObjectiveModule
	{
		...
		private static readonly string localized_noEC = Lib.Color(Localizer.Format("#KERBALISM_Laboratory_NoEC"), Lib.KColor.Yellow);
		...
		private static Status Analyze(Vessel v, string filename, double amount)
		{
			...
			if (completed)
			{
				if(!PreferencesScience.Instance.analyzeSamples)
				{
					...
					Message.Post(Lib.BuildString(Lib.Color(Localizer.Format("#KERBALISM_Laboratory_Analysis"), Lib.KColor.ScienceBlue, true), "\n",
		...
		private void SetStatusText()
		{
			switch (status)
			{
				...
				case Status.NO_RESEARCHER:
					status_txt = Lib.Color(researcher_cs.Warning(), Lib.KColor.Yellow);
					break;
