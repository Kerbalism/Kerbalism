...
namespace KERBALISM
{
	...
	public class SciencePopup
	{
		...
		private void RnDUpdate()
		{
			...
			sb.Append("<pos=15><color=#6DCFF6><b>RnD</b></color><pos=60><color=#6DCFF6><b>flight</b></color><pos=110><color=#6DCFF6><b>value</b></color><pos=155><color=#FFD200><b>retrieved</b></color>\n");

			UnityEngine.Profiling.Profiler.BeginSample("Kerbalism.SciencePopup.RnDUpdate.MainLoop");
			// crew report : 200 ko / 2.49 ms (using keyvaluepair)
			foreach (ObjectPair<int, SituationsBiomesSubject> bodySubjects in GetSubjectsForExperiment(expInfo))
			{
				...
				sb.Append(Lib.Color(body.name, Lib.Kolor.Orange, true));
				sb.Append("\n");

				foreach (ObjectPair<ScienceSituation, BiomesSubject> situationSubjects in bodySubjects.Value)
				{
					sb.Append("<pos=5>");
					sb.Append(Lib.Color(situationSubjects.Key.Title(), Lib.Kolor.Yellow, true));
					sb.Append("\n");

					foreach (ObjectPair<int, List<SubjectData>> biomeSubjects in situationSubjects.Value)
					{

						foreach (SubjectData subject in biomeSubjects.Value)
						{
							sb.Append("<pos=15>");
							sb.Append(Lib.Color(subject.ScienceRetrievedInKSC.ToString("F1"), Lib.Kolor.Science, true));
							sb.Append("<pos=60>");
							sb.Append(Lib.Color(Lib.BuildString("+", subject.ScienceCollectedInFlight.ToString("F1")), Lib.Kolor.Science, true));
							sb.Append("<pos=110>");
							sb.Append(Lib.Color(subject.ScienceMaxValue.ToString("F1"), Lib.Kolor.Science, true));
							sb.Append("<pos=155>");
							sb.Append(Lib.Color(Lib.BuildString(subject.PercentRetrieved.ToString("F1"), " x"), Lib.Kolor.Yellow, true));

							if (biomeSubjects.Key >= 0)
							{
								sb.Append("<pos=200>");
								sb.Append(Lib.Color(body.BiomeMap.Attributes[biomeSubjects.Key].name, Lib.Kolor.LightGrey, true));
							}
							sb.Append("\n");
						}
					}
			...
			UnityEngine.Profiling.Profiler.BeginSample("Kerbalism.SciencePopup.RnDUpdate.Stringbuilder");
			rndContent.SetText(sb.ToString());
			UnityEngine.Profiling.Profiler.EndSample();
			...
			// foreach subject from this experiment that exists in RnD, make sure our own ExperimentInfo exists
			// note : this is ugly and VERY slow but there is no non-ugly around it until we migrate
			// ExperimentInfo to ExperimentData, as a non-volatile object
			foreach (ScienceSubject subject in ResearchAndDevelopment.GetSubjects())
			{
				if (subject.id.StartsWith(moduleOrPrefab.experiment_id))
					Science.GetExperimentInfo(subject.id);
			}

			foreach (ExperimentInfo expInfo in Science.GetExperimentInfos())
			{
				if (expInfo.IsSubject && expInfo.ExperimentId == moduleOrPrefab.experiment_id)
				{
					SubjectInfo subjectInfo = new SubjectInfo();
					subjectInfo.expInfo = expInfo;
					if (expInfo.SubjectExistsInRnD)
					{
						subjectInfo.totalValue = expInfo.SubjectScienceMaxValue;
						subjectInfo.retrieved = expInfo.SubjectScienceRetrievedInKSC;
			...
			{
				rndContent.SetText(Lib.Color("Nothing retrieved yet", Lib.Kolor.Yellow, true));
				return;
			}
			else
			{
				...
				foreach (SubjectInfo si in subjectInfos.Values.OrderBy(p => p.expInfo.SubjectBodyString).ThenBy(p => p.expInfo.SituationString))
				{
					if (currentBody != si.expInfo.SubjectBodyString)
					{
						currentBody = si.expInfo.SubjectBodyString;
						currentSituation = string.Empty;
						sb.Append(Lib.Color(currentBody, Lib.Kolor.Orange, true));
						sb.Append("\n");
					}

					if (currentSituation != si.expInfo.SituationString)
					{
						currentSituation = si.expInfo.SituationString;

						sb.Append("<pos=5>");
						sb.Append(Lib.Color(currentSituation, Lib.Kolor.Yellow, true));
						sb.Append("\n");
					}

					sb.Append("<pos=15>");
					sb.Append(Lib.Color(si.retrieved.ToString("F1"), Lib.Kolor.Science, true));
					sb.Append("<pos=60>");
					sb.Append(Lib.Color(Lib.BuildString("+", si.collected.ToString("F1")), Lib.Kolor.Science, true));
					sb.Append("<pos=110>");
					sb.Append(Lib.Color(si.totalValue > 0.0 ? si.totalValue.ToString("F1") : "?", Lib.Kolor.Science, true));
					sb.Append("<pos=155>");
					sb.Append(Lib.Color(Lib.BuildString(si.percentRetrieved.ToString("F1"), " x"), Lib.Kolor.Yellow, true));

					if (si.expInfo.BiomeString != string.Empty)
					{
						sb.Append("<pos=200>");
						sb.Append(Lib.Color(si.expInfo.BiomeString, Lib.Kolor.LightGrey, true));
					}
					sb.Append("\n");
				}
