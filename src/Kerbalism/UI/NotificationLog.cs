...
namespace KERBALISM
{
	public static class NotificationLog
	{
		...
		{
			p.Title(Lib.BuildString(Lib.Ellipsis(v.vesselName, Styles.ScaleStringLength(40)), " ", Lib.Color("ALL LOGS", Lib.KColor.LightGrey)));
			...
			else
			{
				...
				for (int i = Message.all_logs.Count - 1; i >= 0; --i) //count backwards so most recent is first
				{
					...
					{
						p.AddContent(Lib.Color("ALERT   ", Lib.KColor.Yellow), log.msg.Replace("\n", ". "));
					}
