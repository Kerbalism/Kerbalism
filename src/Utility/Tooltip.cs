...
public sealed class Tooltip
{
  ...
  void get_tooltip()
  {
    ...
    if (Event.current.type == EventType.Repaint)
    {
      ...
      if (tooltip.Length > 0)
      {
        ...
        {
          Styles.tooltip_style.alignment = TextAnchor.MiddleLeft;
          tooltip = tooltip.Replace("<align=left />", "");
        }
        else if (tooltip.IndexOf("<align=right />", StringComparison.Ordinal) != -1)
        {
          Styles.tooltip_style.alignment = TextAnchor.MiddleRight;
          tooltip = tooltip.Replace("<align=right />", "");
        }
        else Styles.tooltip_style.alignment = TextAnchor.MiddleCenter;
      }
    }
  }
  ...
  void render_tooltip(Rect screen_rect)
  {
    ...
    Vector2 tooltip_size = Styles.tooltip_style.CalcSize(tooltip_content);
    tooltip_size.y = Styles.tooltip_style.CalcHeight(tooltip_content, tooltip_size.x);
    ...
    GUILayout.BeginArea(tooltip_rect, Styles.tooltip_container_style);
    GUILayout.Label(tooltip_content, Styles.tooltip_style);
    GUILayout.EndArea();
  }
