...
As an alternative, you can pass these properties to the target from the command line.

#### `BuildDebug`

**`msbuild BuildSystemTargets.xml -v:m -target:BuildDebug -property:[see below]`**

**`DefineConstants`** [comma separated values, default to what is defined in `UserConfigDevEnv.xml`] Compilation constants
...
**`GithubOAuthToken`** : [required string] GitHub OAuth Token for your github account, to generate it, 
...
**`msbuild BuildSystemTargets.xml -v:m -target:ExtractKSPDlls -property:KSPVersion=1.7.3`**
...
**`msbuild BuildSystemTargets.xml -v:m -target:CreateKSPDllProtectedZip -property:KSPVersion=1.7.3;KSPDllPath=Full\Path\To\KSP\DLLS`**

Create a password-protected 7zip archive named KSP-X.X.X.7z from the specified 
