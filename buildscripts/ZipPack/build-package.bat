...
xcopy /y /e "%TargetZipFolder%\GameData\%TargetName%\*" .
xcopy /y %TargetZipFolder%\CHANGELOG.md .
xcopy /y %TargetZipFolder%\License .
xcopy /y %TargetZipFolder%\README.md .

echo.
echo Compressing %TargetName% Release Package...
IF EXIST "%rootPath%%TargetName%*.zip" del "%rootPath%%TargetName%*.zip"
"%scriptPath%7za.exe" a "%TargetZipFolder%\%TargetName%-%Dllversion%.zip" ..\..\GameData
