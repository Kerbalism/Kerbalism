...
echo %TargetName%.dll -^> %TargetName%%KSPversion%.kbin
move /y "%initialWD%\%TargetName%.dll" "%initialWD%\%TargetName%%KSPversion%.kbin" > nul
...
IF %KSPversion% GTR 13 xcopy /y "%initialWD%\%TargetName%%KSPversion%.kbin" "%initialWD%\%TargetName%14.kbin*" > nul
IF %KSPversion% GTR 13 xcopy /y "%initialWD%\%TargetName%%KSPversion%.kbin" "%initialWD%\%TargetName%15.kbin*" > nul
rem copy Bootstrap bins from build directory to GameData
xcopy /y "%initialWD%\%TargetName%*.kbin" "GameData\%TargetName%\*" > nul
...
IF NOT EXIST "package\GameData\%TargetName%\%TargetName%*.kbin" echo **WARNING** %TargetName% bin is missing
