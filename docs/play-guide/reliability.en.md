## MTBF
The [Mean Time Between Failures](https://en.wikipedia.org/wiki/Mean_time_between_failures) is specified per-component and indicates how often it will experience a failure on average.
...
Kerbals can also repair malfunctioned components, provided that they have the necessary specialization and experience level required. A difficulty option can require **repair kits** to be consumed when repairing (off by default).
