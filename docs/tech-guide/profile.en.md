A profile is a named set of rules, supplies and processes. Multiple profiles can coexist in the same install and can be defined anywhere inside the GameData folder. At any time only the one specified in the Profile parameter in settings_ is used.
...
## Process
Processes are 'vessel-wide' resource producers/consumers, with rates that can be influenced by the environment, habitat conditions, and level of resources. All processes will be added to the vessel. To allow players to control a process, use a pseudo-resource as modifier and add ProcessController PartModules.

Rates for processes are per-second, multiplied by the `capacity` of the ProcessController that runs them.
